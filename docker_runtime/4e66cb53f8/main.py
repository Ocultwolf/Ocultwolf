...
### Notas importantes:
- Se asume que has construido y persistido previamente un índice FAISS con tus documentos de documentación. Si no, debes hacerlo primero: cargar docs, dividir texto, crear embeddings y persistir FAISS.
- El agente tiene memoria conversacional (buffer) para mantener contexto entre preguntas.
- Usa OpenAI embeddings y LLM, ajusta nombres de modelos y claves API según tu configuración.
- La función `create_agent` de LangGraph integra memoria, herramientas y prompts de forma idiomática.

Si quieres también te puedo proveer el código para construir y persistir el índice vectorial desde archivos de documentación. ¿Quieres?